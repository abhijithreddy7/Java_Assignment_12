...
import assignment.classes.Student;
import java.util.*;
...
public class Main {
    public static void main(String[] args) {
        ...
        List<String> allDepartments = studentList.stream().map(Student::getEngDepartment).distinct().collect(Collectors.toList());
        ...
        List<String> studentNames = studentList.stream().filter(student -> student.getYearOfEnrollment()>2018).map(Student::getName).collect(Collectors.toList());
        ...
        List<Student> maleStudents = studentList.stream().filter(student ->student.getGender()=="Male" && student.getEngDepartment()=="Computer Science").collect(Collectors.toList());
        ...
        Map<String,Long> studentCount = studentList.stream().collect(Collectors.groupingBy(Student::getGender,Collectors.counting()));
        ...
        Optional<Student> maxPercent = studentList.stream().collect(Collectors.maxBy(Comparator.comparingDouble(Student::getPerTillDate)));
        ...
        Map<String,Double> avgPercentage = studentList.stream().collect(Collectors.groupingBy(Student::getEngDepartment,Collectors.averagingDouble(Student::getPerTillDate)));
        ...
        Optional<Student> youngestMale = studentList.stream().filter(student -> student.getGender()=="Male" && student.getEngDepartment()=="Electronic").min(Comparator.comparing(Student::getAge));
        ...
        Map<String,Long> studentCountInCSDept = studentList.stream().filter(student -> student.getEngDepartment()=="Computer Science").collect(Collectors.groupingBy(Student::getGender,Collectors.counting()));
